...
CREATE TABLE IF NOT EXISTS ind_dep (
  id SMALLINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
  indep_ind_id VARCHAR(63) NOT NULL,
  dep_ind_id VARCHAR(63) NOT NULL,
  CONSTRAINT `fk_indep_ind_ind`
    FOREIGN KEY (indep_ind_id) REFERENCES ind (id)
    ON UPDATE CASCADE
    ON DELETE CASCADE,
  CONSTRAINT `fk_dep_ind_ind`
    FOREIGN KEY (dep_ind_id) REFERENCES ind (id)
    ON UPDATE CASCADE
    ON DELETE CASCADE
) ENGINE = InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_danish_ci;
