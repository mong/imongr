...
if (is.null(check_db(is_test_that = FALSE))) {
  ...
  pool::dbExecute(pool, "ALTER TABLE `vurdering` DROP FOREIGN KEY `fk_vurdering_user`;")
  pool::dbExecute(pool, "ALTER TABLE `vurdering` DROP FOREIGN KEY `fk_vurdering_registry`;")
  pool::dbExecute(
    pool,
    paste(
      "DROP TABLE",
